...
import DeleteTransfert from "./DeleteTransfert";
import moment from "moment";

const HomeTransfert: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://:9001/transferts/${userEmail}`);
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      setTransferts(
        transferts.filter((depense: any) => depense.idTransfert !== id)
      );
    } catch (error) {
  ...
  const handleItemClick = (route) => {
    history.push(`view-transfert${route}`); // Redirige vers la route spécifiée
  };
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>Liste des Transferts</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
            ...
            {transferts.map((transfert) => (
              ...
              >
                <IonItem style={{ flex: "1" }}>
                  ...
                  >
                    {transfert.montant} DT
                  </IonLabel>
                  {"­  "}
                  <IonLabel style={{ marginLeft: "1.2vh" }}>
                    {dateFormat(transfert.dateTransfert)}
                  </IonLabel>
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    <Link to={`/view-transfert/${transfert.idTransfert}/`}>
                      <IonIcon color="success" icon={eyeOutline} />
                    </Link>
                    &nbsp;
                    <DeleteTransfert
                      id={transfert.idTransfert}
